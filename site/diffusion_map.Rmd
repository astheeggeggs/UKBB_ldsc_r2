...
Here we use the squared genetic correlation matrix as the affinity matrix, and compute a Nonlinear dimensionality reduction. We then project this onto the plane using the R package `ForceAtlas`. Points are coloured by the second level of the `path` column in the UK Biobank Data dictionary. Colouring by the final level of the `path` column can be found [here](diffusion_map_final_level.html).
...
p <- div(plot_ly(
  ...
  # Hover text:
  text = atlasproj$name,
  color = ~factor(level2cat), #size = ~carat
  hoverinfo='text',
